...
export default function EventPlan() {
  ...
  const [formData, setFormData] = useState({
    ...
    budgetRows: Array(5).fill().map(() => ({ name: "", quantity: 0, price: 0, total: 0 })),
  ...
  const handleAddTask = useCallback(() => {
    setFormData((prev) => ({
      ...
      tasks: [
        ...prev.tasks,
        { taskName: "", deadline: "", budget: 0, description: "", expanded: false, subtasks: [] },
      ],
    }));
  }, []);
  ...
  const prepareEventData = useCallback(() => {
    ...
    return {
      ...
      Risks: formData.risks
          .filter((risk) => risk.reason.trim() || risk.description.trim() || risk.solution.trim())
          .map((risk, index) => ({
            Name: `Risk ${index + 1}`,
            Reason: risk.reason,
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.startTime || !formData.endTime) return "Thời gian bắt đầu và kết thúc là bắt buộc!";
    ...
    if (!formData.tasks.some((task) => task.taskName.trim())) return "Phải có ít nhất một task!";
    return null;
  }, [formData]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        const eventId = response.data.data?.Id || 0; // Lấy eventId từ phản hồi nếu có
        await axios.post(
        ...
        toast.error(`Tạo sự kiện thất bại với mã trạng thái: ${response.status}`);
      }
    } catch (error) {
      ...
      <div className="working-container">
        ...
        <div className="walkthrough show reveal" ref={modalRef}>
              ...
              <li className="screen active">
                ...
                <Form className="w-75 mx-auto text-start shadow p-4 rounded bg-light">
                  ...
                  <Form.Group className="mb-3 position-relative">
                    <Form.Label>Loại hình sự kiện <span className="text-danger">*</span></Form.Label>
                    <Form.Control
                        as="select"
                        value={formData.categoryEventId}
                        onChange={(e) => handleFormChange("categoryEventId", Number(e.target.value))}
                    >
                      <option value={0} disabled>Chọn loại hình</option>
                  ...
                  <Form.Group className="mb-3">
                    <Form.Label>Địa điểm tổ chức <span className="text-danger">*</span></Form.Label>
                    <Form.Control
                        type="text"
                        ...
                        <div className="row">
                          ...
                          <Form.Group className="col-md-4 mb-3">
                            ...
                            <Form.Control
                                ...
                                onChange={(e) =>
                                    handleTaskChange(i, "budget", e.target.value.replace(/\D/g, ""))
                                }
                            />
                          </Form.Group>
                          <Form.Group className="col-md-4 mb-3">
                        ...
                        <tr key={i}>
                          ...
                          <td>
                            <Form.Control
                                ...
                                onChange={(e) =>
                                    handleBudgetChange(i, "price", e.target.value.replace(/\D/g, ""))
                                }
                            />
                          </td>
                ...
                <Form className="w-75 mx-auto text-start shadow p-4 rounded bg-light">
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control
                        ...
                        onChange={(e) => handlePromotionalPlanChange("before", e.target.value)}
                        style={{ resize: "vertical", minHeight: "100px" }}
                    />
                  </Form.Group>
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control
                        ...
                        onChange={(e) => handlePromotionalPlanChange("during", e.target.value)}
                        style={{ resize: "vertical", minHeight: "100px" }}
                    />
                  </Form.Group>
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control
                        ...
                        onChange={(e) => handlePromotionalPlanChange("after", e.target.value)}
                        style={{ resize: "vertical", minHeight: "100px" }}
                    />
                  </Form.Group>
                </Form>
          ...
          <Button
              ...
              disabled={isLoading}
          >
            {isLoading ? "Đang xử lý..." : index === indexMax() ? "Tạo Sự Kiện" : "Tiếp Theo"}
          </Button>
        </div>
