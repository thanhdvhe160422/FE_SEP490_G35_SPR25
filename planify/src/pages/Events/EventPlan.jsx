...
const Chatbot = ({ onApplyEventData, token }) => {
  ...
  const fetchCategories = async () => {
    ...
    if (!campusId) {
      setChatHistory((prev) => [...prev, { sender: "bot", text: "Không thể xác định campusId từ token!" }]);
      setIsLoading(false);
      return;
    }

    try {
      const response = await axios.get(`https://localhost:44320/api/Categories/${campusId}`, {
      ...
      if (isWaitingForCategory && fetchedCategories.length > 0) {
        setChatHistory((prev) => [
          ...prev,
          { sender: "bot", text: "Cảm ơn bạn! Bây giờ hãy chọn một danh mục sự kiện:" },
          { sender: "bot", categories: fetchedCategories },
        ]);
      } else if (fetchedCategories.length === 0) {
        setChatHistory((prev) => [...prev, { sender: "bot", text: "Không có danh mục nào khả dụng!" }]);
      }
    } catch (error) {
      console.error("Error fetching categories:", error);
      setChatHistory((prev) => [...prev, { sender: "bot", text: "Lỗi khi tải danh mục sự kiện!" }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCategorySelect = async (category) => {
    ...
    setChatHistory((prev) => [
      ...prev,
      { sender: "bot", text: `Bạn đã chọn danh mục: ${category.categoryEventName}` },
      { sender: "bot", text: "Vui lòng đợi chút, tôi đang tạo kế hoạch cho bạn..." },
    ]);
    ...
    try {
      ...
      setChatHistory((prev) => [
        ...prev.filter((msg) => msg.text !== "Vui lòng đợi chút, tôi đang tạo kế hoạch cho bạn..."),
        { sender: "bot", data },
        { sender: "bot", text: "Bạn có muốn điền kế hoạch này vào form không?" },
  ...
  const handleConfirmation = (apply) => {
    if (apply) {
      ...
      setChatHistory((prev) => [...prev, { sender: "bot", text: "Dữ liệu đã được điền vào form!" }]);
    } else {
      setChatHistory((prev) => [...prev, { sender: "bot", text: "Đã hủy điền dữ liệu." }]);
    }
  ...
  const renderEventSuggestion = (data) => {
    ...
    const categoryName = categories.find((cat) => cat.id === data.CategoryEventId)?.categoryEventName || "Không xác định";
    ...
    return (
        <div className="event-suggestion">
          ...
          <p><strong>Thời gian:</strong> {new Date(data.StartTime).toLocaleString("vi-VN")} - {new Date(data.EndTime).toLocaleString("vi-VN")}</p>
          <p><strong>Ngân sách:</strong> {Number(data.AmountBudget).toLocaleString("vi-VN")} VNĐ</p>
          <p><strong>Danh mục:</strong> {categoryName}</p>
          <p><strong>Địa điểm:</strong> {data.Placed || "Không xác định"}</p>
          <p><strong>Khẩu hiệu:</strong> {data.SloganEvent || "Không có khẩu hiệu"}</p>
          <p><strong>Mục tiêu:</strong> {data.Goals || "Không có mục tiêu"}</p>
          <p><strong>Đối tượng tham gia:</strong> {data.TargetAudience || "Không xác định"}</p>
          <p><strong>Số lượng người tham gia:</strong> {data.SizeParticipants || "Không xác định"}</p>
          <p><strong>Cách đo lường thành công:</strong> {data.MeasuringSuccess || "Không xác định"}</p>
          <p><strong>Quy trình giám sát:</strong> {data.MonitoringProcess || "Không xác định"}</p>
          <p><strong>Kế hoạch quảng bá:</strong> {data.PromotionalPlan || "Không có kế hoạch quảng bá"}</p>

          {data.Tasks && data.Tasks.length > 0 && (
              <div>
                <div className="collapsible-header" onClick={() => toggleSection("tasks")}>
                    ...
                    <div className="collapsible-content">
                      {data.Tasks.map((task, idx) => (
                          <div key={idx} className="task-item">
                            ...
                            <p>Hạn chót: {new Date(task.Deadline).toLocaleString("vi-VN")}</p>
                            <p>Ngân sách: {Number(task.AmountBudget).toLocaleString("vi-VN")} VNĐ</p>
                            {task.SubTasks && task.SubTasks.length > 0 && (
                                <div>
                                  <h5>Nhiệm vụ con:</h5>
                                  {task.SubTasks.map((subTask, subIdx) => (
                                      <div key={subIdx} style={{ marginLeft: "20px" }}>
                                        <p><strong>{subTask.SubTaskName}</strong></p>
                                        <p>{subTask.SubTaskDescription || "Không có mô tả"}</p>
                                        <p>Hạn chót: {new Date(subTask.Deadline).toLocaleString("vi-VN")}</p>
                                        <p>Ngân sách: {Number(subTask.AmountBudget).toLocaleString("vi-VN")} VNĐ</p>
                                      </div>
                                  ))}
                                </div>
                            )}
          ...
          {data.BudgetRows && data.BudgetRows.length > 0 && (
              <div>
                <div className="collapsible-header" onClick={() => toggleSection("budgetRows")}>
                    ...
                    <div className="collapsible-content">
                      {data.BudgetRows.map((budget, idx) => (
                          <div key={idx} className="budget-item">
                            ...
                            <p>Giá mỗi đơn vị: {Number(budget.Price).toLocaleString("vi-VN")} VNĐ</p>
                            <p>Tổng: {Number(budget.Total).toLocaleString("vi-VN")} VNĐ</p>
                          </div>
                      ))}
                    </div>
                )}
          ...
          {data.Risks && data.Risks.length > 0 && (
              <div>
                <div className="collapsible-header" onClick={() => toggleSection("risks")}>
          ...
          {data.Activities && data.Activities.length > 0 && (
              <div>
                <div className="collapsible-header" onClick={() => toggleSection("activities")}>
                          ...
                          {chat.data ? (
                              ...
                              <div className="category-buttons">
                                {chat.categories.map((cat, i) => (
                                    <button key={i} onClick={() => handleCategorySelect(cat)}>
                              ...
                              <div className="confirmation-buttons">
                                <button onClick={() => handleConfirmation(true)}>Có</button>
                                <button onClick={() => handleConfirmation(false)}>Không</button>
                              </div>
                          ) : (
                              chat.text
                          )}
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.message || "Error while getting directory.");
      }
    };
    fetchCategories();
...
export default function EventPlan() {
  ...
  const handleApplyEventData = (eventData) => {
    ...
    setFormData((prev) => ({
      ...
      startTime: eventData.StartTime ? new Date(eventData.StartTime).toISOString().slice(0, 16) : prev.startTime,
      endTime: eventData.EndTime ? new Date(eventData.EndTime).toISOString().slice(0, 16) : prev.endTime,
      ...
      tasks: eventData.Tasks && eventData.Tasks.length > 0
          ...
          : prev.tasks,
      budgetRows: eventData.BudgetRows && eventData.BudgetRows.length > 0
          ...
          : prev.budgetRows,
      risks: eventData.Risks && eventData.Risks.length > 0
          ...
          : prev.risks,
      activities: eventData.Activities && eventData.Activities.length > 0
  ...
  const handleSubtaskChange = useCallback((taskIndex, subIndex, field, value) => {
    setFormData((prev) => {
      const updated = [...prev.tasks];
      updated[taskIndex].subtasks[subIndex][field] = field === "amount" ? Number(value) : value;
      return { ...prev, tasks: updated };
    });
  }, []);
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (e.key === "ArrowLeft" && index > 0) setIndex((i) => Math.max(0, i - 1));
    else if (e.key === "ArrowRight" && index < indexMax()) setIndex((i) => Math.min(i + 1, indexMax()));
  ...
  useEffect(() => {
    if (screens.current.length > 0 && dots.current.length > 0) updateScreen(index);
  }, [index, updateScreen]);
          ...
          .map((task) => ({
            ...
            SubTasks: task.subtasks
                ...
                .map((subtask) => ({
                  ...
                  StartTime: currentTime,
                  Deadline: subtask.deadline ? new Date(subtask.deadline).toISOString() : null,
                  Budget: subtask.amount,
                })),
          })),
      Risks: formData.risks
          .filter((risk) => risk.name.trim() && (risk.reason.trim() || risk.description.trim() || risk.solution.trim()))
          .map((risk) => ({
            Name: risk.name,
            Reason: risk.reason,
            Solution: risk.solution,
  ...
  const handleUploadImages = async (eventId, token) => {
    ...
    try {
      await axios.post("https://localhost:44320/api/Events/upload-image", formData, {
        headers: {
      ...
      enqueueSnackbar("Image upload failed. You can try uploading manually.", { variant: "warning" });
    }
  };
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.startTime || !formData.endTime) return "Start time and end time are required!";
    if (new Date(formData.startTime) >= new Date(formData.endTime)) return "Start time must be earlier than end time!";
    if (!formData.categoryEventId) return "Event category is required!";
    if (!formData.placed) return "Event location is required!";
    if (!formData.tasks.some((task) => task.taskName.trim())) return "At least one task is required!";
    if (!formData.activities.some((activity) => activity.name.trim())) return "At least one activity with a name is required!";
    return null;
  }, [formData]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.post("https://localhost:44320/api/Events/create", eventData, {
        headers: {
      ...
      if (response.status === 201) {
        await axios.post("https://localhost:44320/api/SendRequest", { eventId }, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        ...
        enqueueSnackbar(`Failed to create event with status code: ${response.status}`, { variant: "error" });
      }
    } catch (error) {
      enqueueSnackbar(error.response?.data?.message || "Error while creating event!", { variant: "error" });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await handleUploadImages(response.data.result?.id, localStorage.getItem("token"));
          ...
          enqueueSnackbar(error.response?.data?.message || "Error while saving draft!", { variant: "error" });
          console.error("Lỗi chi tiết:", error);
        } finally {
          setIsLoading(false);
        }
  ...
  const totalBudget = formData.budgetRows.reduce((sum, row) => sum + row.total, 0);
        ...
        <div className="working-container">
          ...
          <div className="walkthrough show reveal" ref={modalRef}>
                  ...
                  <Form className="w-75 mx-auto text-start shadow p-4 rounded bg-light">
                    ...
                    <Form.Group className="mb-3 position-relative">
                      ...
                      <Form.Control
                          ...
                          onChange={(e) => handleFormChange("categoryEventId", Number(e.target.value))}
                      >
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                          ...
                          onChange={(e) => handleFormChange("sloganEvent", e.target.value)}
                  ...
                  <Form className="w-75 mx-auto text-start shadow p-4 rounded bg-light">
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                          ...
                          onChange={(e) => handleFormChange("measuringSuccess", e.target.value)}
                      />
                    </Form.Group>
                  </Form>
                </li>
                ...
                <li className="screen">
                  ...
                  <div className="w-100 mt-3">
                    {formData.tasks.map((task, i) => (
                        <div key={i} className="mb-4 p-4 border rounded bg-light shadow">
                          <div className="d-flex align-items-center justify-content-between mb-3">
                            <div className="d-flex align-items-center gap-2">
                              <Button variant="link" className="p-0" onClick={() => toggleExpand(i)}>
                                {task.expanded ? <FaChevronDown /> : <FaChevronRight />}
                              </Button>
                              <Form.Control
                                  type="text"
                                  value={task.taskName}
                                  onChange={(e) => handleTaskChange(i, "taskName", e.target.value)}
                                  placeholder="Task name"
                                  className="fw-bold"
                              />
                            </div>
                            <Button variant="danger" size="sm" onClick={() => handleRemoveTask(i)}>
                              ✕
                            </Button>
                          </div>
                          ...
                          <div className="row">
                            ...
                            <Form.Group className="col-md-4 mb-3">
                              ...
                              <Form.Control
                                  ...
                                  onChange={(e) => handleTaskChange(i, "budget", e.target.value.replace(/\D/g, ""))}
                              />
                            </Form.Group>
                            <Form.Group className="col-md-4 mb-3">
                              <Form.Label>Description</Form.Label>
                                    ...
                                    <div key={j} className="row mb-3 align-items-start">
                                      <Form.Group className="col-md-3">
                                        ...
                                        <Form.Control
                                            ...
                                            onChange={(e) => handleSubtaskChange(i, j, "subtaskName", e.target.value)}
                                      ...
                                      <Form.Group className="col-md-3">
                                        ...
                                        <Form.Control
                                            ...
                                            value={sub.deadline}
                                            min={minDateTime}
                                            onChange={(e) => handleSubtaskChange(i, j, "deadline", e.target.value)}
                                        />
                                      </Form.Group>
                                      <Form.Group className="col-md-2">
                                        ...
                                        <Form.Control
                                            ...
                                            onChange={(e) => handleSubtaskChange(i, j, "amount", e.target.value.replace(/\D/g, ""))}
                                        />
                                      </Form.Group>
                                      <Form.Group className="col-md-3">
                                        <Form.Label>Description</Form.Label>
                                        <Form.Control
                                            ...
                                            onChange={(e) => handleSubtaskChange(i, j, "description", e.target.value)}
                                            style={{ resize: "vertical", minHeight: "80px" }}
                                        />
                                      </Form.Group>
                                      <div className="col-md-1 text-end">
                                        <Button variant="danger" size="sm" onClick={() => handleRemoveSubtask(i, j)}>
                                          ✕
                                        </Button>
                                      </div>
                                    </div>
                                ))}
                                <Button variant="outline-primary" size="sm" onClick={() => handleAddSubtask(i)}>
                                  + Create Sub-task
                                </Button>
                              </div>
                          )}
                      ...
                      {formData.budgetRows.map((row, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              <Form.Control
                                  value={row.name}
                                  onChange={(e) => handleBudgetChange(i, "name", e.target.value)}
                            ...
                            <td>
                              <Form.Control
                                  ...
                                  onChange={(e) => handleBudgetChange(i, "price", e.target.value.replace(/\D/g, ""))}
                              />
                            </td>
                            <td className="d-flex align-items-center gap-2">
                              <Button variant="outline-secondary" size="sm" onClick={() => updateQuantity(i, -1)}>
                                -
                              </Button>
                              <Form.Control
                                  type="number"
                                  value={row.quantity}
                                  onChange={(e) => handleBudgetChange(i, "quantity", e.target.value)}
                            ...
                            <td>
                              <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    updated.splice(i, 1);
                                    setFormData((prev) => ({ ...prev, budgetRows: updated }));
                                  }}
                              >
                                ✕
                              </Button>
                        ...
                        <td colSpan="6" className="text-center">
                          <Button
                              ...
                              onClick={() =>
                                  setFormData((prev) => ({
                                    ...prev,
                                    budgetRows: [...prev.budgetRows, { name: "", quantity: 0, price: 0, total: 0 }],
                                  }))
                              }
                          >
                            + Create Extra Cost
                  ...
                  <Form className="w-75 mx-auto text-start shadow p-4 rounded bg-light">
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                          ...
                          onChange={(e) => handleFormChange("monitoringProcess", e.target.value)}
                          style={{ resize: "vertical", minHeight: "100px" }}
                      />
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                          ...
                          onChange={(e) => handleFormChange("promotionalPlan", e.target.value)}
                          style={{ resize: "vertical", minHeight: "100px" }}
                      />
                    </Form.Group>
                  </Form>
                          ...
                          <tr key={i}>
                            ...
                            <td>
                              <Form.Control
                                  ...
                                  style={{ resize: "vertical", minHeight: "100px" }}
                                  value={activity.name}
                                  onChange={(e) => handleActivityChange(i, "name", e.target.value)}
                              />
                            </td>
                            <td>
                              <Form.Control
                                  ...
                                  onChange={(e) => handleActivityChange(i, "content", e.target.value)}
                              />
                            </td>
                            <td>
                              <Button variant="danger" size="sm" onClick={() => handleRemoveActivity(i)}>
                                ✕
                              </Button>
                            </td>
                          </tr>
                      ...
                      {formData.risks.map((risk, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              <Form.Control
                                  value={risk.name}
                                  onChange={(e) => handleRiskChange(i, "name", e.target.value)}
                              />
                            </td>
                            <td>
                              <Form.Control
                                  value={risk.reason}
                                  onChange={(e) => handleRiskChange(i, "reason", e.target.value)}
                              />
                            </td>
                            <td>
                              <Form.Control
                                  value={risk.description}
                                  onChange={(e) => handleRiskChange(i, "description", e.target.value)}
                              />
                            </td>
                            <td>
                              <Form.Control
                                  value={risk.solution}
                                  onChange={(e) => handleRiskChange(i, "solution", e.target.value)}
                              />
                            </td>
                            <td>
                              <Button variant="danger" size="sm" onClick={() => handleRemoveRisk(i)}>
                                ✕
                              </Button>
                            </td>
                          </tr>
                      ...
                      <Row>
                        <Col xs={3} className="mb-3">
                          <label
                              ...
                              style={{ cursor: "pointer", aspectRatio: "16/9", minHeight: "120px" }}
                          >
                            <FaPlus />
                            <input
                                type="file"
                        ...
                        {selectedImages.map((file, index) => (
                            <Col xs={3} key={index} className="position-relative mb-3">
                              <img
                                  ...
                                  style={{ aspectRatio: "16/9", minHeight: "120px", objectFit: "cover" }}
                              />
                              <Button
                                  variant="danger"
                                  size="sm"
            ...
            <Button
                ...
                onClick={() => (index === indexMax() ? handleSubmit() : setIndex((i) => i + 1))}
                disabled={isLoading}
            >
              {isLoading ? "Processing..." : index === indexMax() ? "Create Event" : "Next"}
            </Button>
          </div>
          <div className="shade" ref={shadeRef} onClick={closeModal}></div>
        </div>
        <Chatbot onApplyEventData={handleApplyEventData} token={localStorage.getItem("token")} />
      </>
  );
}